...
''' + JS + '''
(function(){var NL=String.fromCharCode(10);
    window._origShow = window.showStockInfo;
    window.showStockInfo = function(el) {
        var name = el.getAttribute("data-name") || "";
        var code = el.getAttribute("data-code") || "";
        var tracked = el.getAttribute("data-tracked") === "1";
        if (tracked) { return window._origShow(el); }
        ...
        fetch(u,{headers:{"Authorization":"Bearer "+GH_TOKEN,"Accept":"application/vnd.github+json"}}).then(function(r){return r.json();}).then(function(d){
            var lines = atob(d.content).split(NL).map(function(l){return l.trim();}).filter(function(l){return /^[0-9]{6}$/.test(l);});
            if (lines.indexOf(code) >= 0) { toast.textContent = name + "(" + code + ") 已在关注列表中"; toast.className = "toast show"; }
            else { lines.push(code); var nc = lines.join(NL) + NL; return fetch(u,{method:"PUT",headers:{"Authorization":"Bearer "+GH_TOKEN,"Accept":"application/vnd.github+json","Content-Type":"application/json"},body:JSON.stringify({message:"add "+code+" "+name,content:btoa(nc),sha:d.sha})}).then(function(pr){if(pr.ok){toast.textContent="✅ "+name+"("+code+") 已添加！明天日报将包含此股票";toast.className="toast success show";TRACKED_SET[code]=true;el.setAttribute("data-tracked","1");var sp=el.querySelector("span");if(sp){sp.textContent="已关注";sp.className="si-badge";sp.style.cssText="";}}else{throw new Error("API");}}); }
    ...
    if (sr) {
        var fixBadges = function() {
            var items = sr.querySelectorAll(".search-item");
